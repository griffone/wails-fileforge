...
func TestConvertToolBatchHappy(t *testing.T) {
	tmpDir := t.TempDir()
	...
	for _, item := range items {
		...
		if !strings.HasPrefix(item.OutputPath, tmpDir) {
			t.Fatalf("expected outputPath inside outputDir, got %s", item.OutputPath)
		}
	}
	if runner.runCount != 2 {
		t.Fatalf("expected ffmpeg to run twice, got %d", runner.runCount)
	}
}

func TestConvertToolBatchInvalidMissingOutputDir(t *testing.T) {
	tmpDir := t.TempDir()
	inputPath := filepath.Join(tmpDir, "input.mp4")
	if err := os.WriteFile(inputPath, []byte("fixture"), 0o644); err != nil {
		t.Fatalf("write input: %v", err)
	...
	if err.Code != engine.ErrorCodeVideoValidation {
		t.Fatalf("expected %s, got %s", engine.ErrorCodeVideoValidation, err.Code)
	}
}

func TestConvertToolBatchValidateExecuteParity(t *testing.T) {
	tmpDir := t.TempDir()
	inputPath := filepath.Join(tmpDir, "input.mkv")
	if err := os.WriteFile(inputPath, []byte("fixture"), 0o644); err != nil {
		t.Fatalf("write input: %v", err)
	...
	conflictingOutput := filepath.Join(tmpDir, "input_converted.webm")
